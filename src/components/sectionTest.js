...
export default class SectionTest {
    ...
    _renderIntro() {
        ...
        card.innerHTML = `
      <h1 style="color: var(--accent-secondary); margin-bottom: 0;">Sektionsprov: ${this.section.title}</h1>
      <p style="margin-top: var(--spacing-sm); color: var(--text-muted);">
        Visa vad du har lärt dig! Du måste svara rätt på minst <strong>90%</strong> av frågorna för att låsa upp nästa sektion.
      </p>
      ...
      <div style="background: rgba(255,255,255,0.05); padding: var(--spacing-lg); border-radius: var(--border-radius-sm); margin: var(--spacing-lg) 0;">
        <ul style="list-style-position: inside; color: var(--text-main);">
          <li>Antal frågor: <strong>${this.questions.length}</strong></li>
          <li>Krav för godkänt: <strong>90%</strong> (Minst ${Math.ceil(this.questions.length * 0.9)} rätt)</li>
          <li>Du kan göra provet hur många gånger du vill.</li>
        </ul>
      </div>
      
      <button id="start-test-btn" class="btn-primary" style="background: var(--accent-secondary); font-size: 1.1rem; padding: 1rem 2rem;">Starta Provet Nu</button>
    `;
    ...
    _renderActiveQuestion() {
        ...
        const progressText = `Fråga ${this.currentQuestionIndex + 1} av ${this.questions.length}`;
      ...
      <div class="lesson-navigation" style="border-top: none; padding-top: 0;">
        <button id="test-prev-btn" class="option-btn" style="width: auto; visibility: ${this.currentQuestionIndex > 0 ? 'visible' : 'hidden'};">← Föregående</button>
        <button id="test-next-btn" class="btn-primary" style="background: var(--accent-secondary);" disabled>
          ${this.currentQuestionIndex === this.questions.length - 1 ? 'Lämna in prov' : 'Nästa fråga →'}
        </button>
      </div>
    `;
    ...
    _renderResults() {
      ...
      <h1 style="color: ${passed ? 'var(--accent-success)' : 'var(--accent-warning)'};">
        ${passed ? 'Provet avklarat!' : 'Nästan där!'}
      </h1>
      <p style="font-size: 1.2rem; max-width: 100%;">Du fick <strong>${percentage}%</strong> (${correctCount} av ${this.questions.length} rätt).</p>
      ...
      <div style="background: rgba(255,255,255,0.05); text-align: left; padding: var(--spacing-lg); border-radius: var(--border-radius-sm); margin: var(--spacing-lg) 0;">
        <h3 style="margin-bottom: var(--spacing-md);">Dina svar:</h3>
      ...
      <div style="display: flex; gap: var(--spacing-md); justify-content: center; margin-top: var(--spacing-xl);">
        <button id="retry-btn" class="option-btn" style="width: auto;">Gör om provet</button>
        ${passed ? `<button id="continue-btn" class="btn-primary" style="background: var(--accent-success);">Fortsätt till nästa sektion →</button>` : ''}
      </div>
    `;
